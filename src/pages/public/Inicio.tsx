      ...
      <div className="px-5 pt-2 pb-12 flex flex-col gap-3 max-w-sm mx-auto w-full">
        {PAGES.map((page) => (
          <button
            ...
            className="w-full bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/40 backdrop-blur-sm rounded-2xl px-5 py-4 flex items-center gap-4 transition-colors text-left"
          >
            <span className="text-3xl">{page.icon}</span>
            <div className="flex-1 min-w-0">
              <p className="text-white font-semibold text-base">{page.label}</p>
              <p className="text-white/60 text-sm mt-0.5">{page.description}</p>
            </div>
        ...
        <button
          ...
          className="mt-4 text-white/30 hover:text-white/50 text-xs text-center transition-colors"
        >
          Acceso staff
        </button>
      </div>
